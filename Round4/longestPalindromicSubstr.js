...
function findLongestPalindromeSubstr(str, start = 0, end = str.length - 1) {
  ...
  const palindromeExcludeStart = findLongestPalindromeSubstr(
    str,
    start + 1,
    end
  );
  const palindromeExcludeEnd = findLongestPalindromeSubstr(str, start, end - 1);

  return palindromeExcludeStart.length > palindromeExcludeEnd.length
    ? palindromeExcludeStart
    : palindromeExcludeEnd;
}
