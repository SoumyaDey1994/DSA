 ...
 */
function findUniqueCombinationForTagetSum(candidates, target) {
  ...
  backtrack(candidates, target, start, currSum, subList, result);
  return result.map((group) => `[${group}]`);
}

function backtrack(candidates, target, start, currSum, subList, result) {
...
let candidates = [10, 1, 2, 7, 6, 1, 5],
  target = 8;
console.log(
  `Output is: [${findUniqueCombinationForTagetSum(candidates, target)}]`,
);
