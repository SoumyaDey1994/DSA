...
function findKthHeighestElement(nums, k) {
  ...
  function findElement(input, rank) {
    const left = [],
      right = [];
    const pivot = input[0];
    ...
    for (let i = 1; i < input.length; i++) {
      const item = input[i];
      if (item > pivot) {
        left.push(item); // left consist greater elements
      } else {
        right.push(item); // right consist smaller elements
      }
    }
    ...
    if (pivotIndex === rank) {
      ...
      return findElement(right, rank - pivotIndex);
    }
  }
...
((nums = [5, 3, 1, 8, 2, 9, 11, 15, 6, 4]), (k = 8));
output = findKthHeighestElement(nums, k);
console.log(`${k}th heighest element of [${nums}] is: ${output}`);
