...
function findWaysToPaintHouses(noOfHouses, noOfColors, firstColor, lastColor) {
  ...
  const prevHourseColor = firstColor;
  const startHouse = 1;
  const ways = findWays(
    startHouse,
    noOfHouses,
...
function findWays(
  ...
  if (startHouse === noOfHouses - 1) {
    return prevHourseColor === lastColor ? noOfColors - 1 : noOfColors - 2;
  }
  ...
  for (let i = 1; i <= noOfColors; i++) {
    if (i !== lastColor) {
      ways =
        ways + findWays(startHouse + 1, noOfHouses, noOfColors, i, lastColor);
    }
  }
