...
function removeDuplicates(nums) {
  ...
  for (let i = 1; i < nums.length; i++) {
    if (nums[i] !== nums[countOfUniques]) {
      //   console.log(
      //     `SWAP: Nums[i] = ${nums[i]}, Nums[countOfUniques]: ${nums[countOfUniques]}`,
      //   );
      countOfUniques++;

      const temp = nums[i];
      nums[i] = nums[countOfUniques];
      nums[countOfUniques] = temp;
    }
  }

  for (let i = countOfUniques + 1; i < nums.length; i++) {
    nums[i] = "_";
  }

  return { count: countOfUniques + 1, nums };
}
