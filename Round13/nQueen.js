...
function placeNQueens(n) {
  ...
  const result = [];
  const startRow = 0;
  backtrack(n, startRow, board, result);
  return result;
}

function backtrack(n, startRow, board, result) {
  if (startRow === n) {
  ...
  for (let col = 0; col < n; col++) {
    const isQueenSafe = isSafe(startRow, col, n, board);
    if (isQueenSafe) {
      board[startRow][col] = "Q";
      backtrack(n, startRow + 1, board, result);
      board[startRow][col] = ".";
    }
  }
}
