...
function findMinDistance(stalls, cows) {
  if (!stalls || stalls.length === 0 || cows === 0) return;

  stalls.sort((s1, s2) => s1 - s2); // sort in ASC order
  let min = 1,
  ...
  while (min <= max) {
    ...
    const isPossible = checkIfPossibleToPlace(stalls, cows, pivot);
    if (isPossible) {
      result = pivot;
      min = pivot + 1;
    } else {
...
function checkIfPossibleToPlace(stalls, cows, pivot) {
