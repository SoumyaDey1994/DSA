...
function printMatrixSpiralOrder(arr) {
  ...
  while (left <= right && top <= bottom) {
    if (direction === 0) {
      // keep top same, move left to right, incr top
    ...
    if (direction === 1) {
      // keep right same, move top to bottom, decr right
    ...
    if (direction === 2) {
      // keep bottom same, move right to left, decr bottom
    ...
    if (direction === 3) {
      // keep left same, move bottom to top, incr left
