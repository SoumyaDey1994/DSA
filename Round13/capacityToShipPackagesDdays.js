 ...
 *              [3,2] | [2,4] | [1,4]
 */
function findMinCapacity(weights, d) {
  if (!weights || weights.length === 0 || d === 0) return;

  let minCapacity = Math.max(...weights);
  let maxCapacity = weights.reduce((acc, curr) => acc + curr, 0);
  let result = -1;

  while (minCapacity <= maxCapacity) {
    const pivot = Math.floor((minCapacity + maxCapacity) / 2);
    const canShip = canShipWeights(weights, pivot, d);
    if (canShip) {
      result = pivot;
      maxCapacity = pivot - 1;
    } else {
...
function canShipWeights(weights, pivot, d) {
  let countOfDays = 1,
    sumOfWeights = 0;

  for (let i = 0; i < weights.length; i++) {
    if (sumOfWeights + weights[i] > pivot) {
      countOfDays++;
      sumOfWeights = 0;
    }
