...
function generateSpiralMatrix(n) {
  ...
  let direction = 0,
    counter = 1;
  const output = new Array(n).fill().map(() => new Array(n).fill(null));

  while (left <= right && top <= bottom) {
    if (direction === 0) {
      // keep top same, move from left to right, incr top
    ...
    if (direction === 0) {
      // keep right same, move from top to bottom, decr right
    ...
    if (direction === 0) {
      // keep bottom same, move from right to left, decr bottom
