 ...
 */
function canSegmentWord(str, wordDict) {
  if (!str || str.length === 0) return false;

  const wordSet = new Set(wordDict);
  const aux = new Array(str.length + 1).fill(false);
  aux[0] = true;

  for (let i = 1; i <= str.length; i++) {
    for (let j = 0; j < i; j++) {
      const subStr = str.substring(i, j);
      if (aux[j] && wordSet.has(subStr)) {
        aux[i] = true;
      }
    }
  }
...
let str = "leetcode",
  words = ["leet", "code"];
console.log(
  `${str} can be segmented into [${words}]: ${canSegmentWord(str, words)}`,
);
...
str = "cars";
wordList = ["car", "ca", "rs"];
output = canSegmentWord(str, wordList);
