...
function findNoOfWaysToGetChange(coins, amount) {
  ...
  const memo = new Map();
  function getNoOfWays(start, remaining) {
    ...
    const includeStart = getNoOfWays(start, remaining - coins[start]);
    const excludeStart = getNoOfWays(start + 1, remaining);

    const result = includeStart + excludeStart;
    memo.set(key, result);
    return result;
  }
