...
function findWaysToPaintHouses(noOfHouses, noOfColors, firstColor, lastColor) {
  ...
  let prevColor = firstColor;
  const ways = getNoOfWays(2, noOfHouses, noOfColors, prevColor, lastColor); // start with house 2 as house1 is already pre-decided
  return ways;
}
...
function getNoOfWays(currHouse, noOfHouses, noOfColors, prevColor, lastColor) {
  ...
  let ways = 0;
  // loop over all colors with next house
  for (let color = 1; color <= noOfColors; color++) {
    if (color !== prevColor) {
      ways =
        ways +
        getNoOfWays(currHouse + 1, noOfHouses, noOfColors, color, lastColor);
    }
  }
...
console.log(
  `No of ways to paint ${noOfHouses} houses with ${noOfColors} are: ${noOfWays}`,
);
