 ...
 * Note: Water at water[i] = min(maxLeft[i], maxRight[i]) - height[i]
 * Example 1:
...
function findTotalWaterTrapped(heights) {
    ...
    while(left < right) {
        if(maxLeft < maxRight) {
            ...
            totalWaterTrapped = totalWaterTrapped + Math.max(0, maxLeft - heights[left]);
        } else {
            right--;
            maxRight = Math.max(maxRight, heights[right]);
            totalWaterTrapped = totalWaterTrapped + Math.max(0, maxRight - heights[right]);
        }
    }
