...
function findIntersection(nums1, nums2) {
  if (!nums1 || nums1.length === 0) return [];
  if (!nums2 || nums2.length === 0) return [];
  ...
  for (const number of nums2) {
    if (freqMap.has(number)) {
      result.push(number);
      // reduce count or remove from map
      const count = freqMap.get(number);
      if (count === 1) {
        freqMap.delete(number);
      } else {
        freqMap.set(number, count - 1);
      }
    }
  }
