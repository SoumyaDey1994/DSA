...
function findMaxAmountRobbed(input) {
  ...
  function findAmount(start) {
    if (start === input.length - 1) return input[input.length - 1];
    if (start >= input.length) return 0;

    if (memo.has(start)) return memo.get(start);

    const amountIncludingStart = input[start] + findAmount(start + 2);
    const amountExcludingStart = findAmount(start + 1);

    const result =
      amountIncludingStart > amountExcludingStart
        ? amountIncludingStart
        : amountExcludingStart;
