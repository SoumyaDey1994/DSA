...
function findNextPermutation(input) {
  ...
  if (indexToBeSwapped === -1) return input.reverse();

  for (let j = indexToBeSwapped + 1, k = input.length - 1; j < k; j++, k--) {
    const temp = input[j];
    input[j] = input[k];
    input[k] = temp;
  }

  //   console.log("input post reverse: ", input);

  for (let i = indexToBeSwapped + 1; i < input.length; i++) {
