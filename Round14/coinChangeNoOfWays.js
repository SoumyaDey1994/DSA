...
function findNoOfWaysToGetChange(coins, amount) {
  if (!coins || coins.length === 0 || amount === 0) return;

  const memo = new Map();

  function findWays(start, remaining) {
    // console.log(start, remaining);

    if (remaining === 0) return 1;
    if (remaining < 0 || start >= coins.length) return 0;

    const memoKey = `${start}-${remaining}`;
    if (memo.has(memoKey)) return memo.get(memoKey);

    const waysIncludingCurrentCoin = findWays(start, remaining - coins[start]);
    const waysExcludingCurrentCoin = findWays(start + 1, remaining);

    const result = waysIncludingCurrentCoin + waysExcludingCurrentCoin;
