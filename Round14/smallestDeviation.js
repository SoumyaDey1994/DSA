...
function findSmallestDeviation(integers) {
  if (!integers || integers.length === 0) return;

  integers = integers.map((num) => (num % 2 === 1 ? num * 2 : num));
  integers.sort((num1, num2) => num1 - num2);
  let smallestDeviation = Infinity;

  while (integers[integers.length - 1] % 2 === 0) {
    lastElement = integers.pop();
    lastElement = lastElement / 2;
    integers.push(lastElement);
    integers.sort((num1, num2) => num1 - num2);

    let maxVal = integers[integers.length - 1],
      minVal = integers[0];
    smallestDeviation = Math.min(smallestDeviation, maxVal - minVal);
  }
