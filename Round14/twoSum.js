...
function find2SumIndices(nums, target) {
  ...
  for (let i = 0; i < nums.length; i++) {
    ...
    const complementVal = target - nums[i];
    if (complementMap.has(complementVal)) {
      return [i, complementMap.get(complementVal)];
    }

    complementMap.set(currVal, i);
  }
...
let targetSum = 9;
console.log(
  `TwoSum indices of [${nums}] for sum ${targetSum} are: [${find2SumIndices(
    nums,
    targetSum,
  )}]`,
);
