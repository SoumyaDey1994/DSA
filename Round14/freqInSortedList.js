...
function findFrequency(arr, target) {
  ...
  const leftMostIndex = getIndex(arr, target, true);
  const rightMostIndex = getIndex(arr, target, false);

  return leftMostIndex !== -1 ? rightMostIndex - leftMostIndex + 1 : 0;
}

function getIndex(arr, target, isLeft) {
  let left = 0,
    right = arr.length - 1;
  let result = -1;
