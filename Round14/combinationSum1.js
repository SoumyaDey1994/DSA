...
function findCombinationsForTargetSum(candidates, target) {
  ...
  const startIndex = 0,
    currSum = 0;
  backtrack(candidates, target, startIndex, currSum, [], result);
...
function backtrack(candidates, target, startIndex, currSum, subList, result) {
  ...
  for (let i = startIndex; i < candidates.length; i++) {
    ...
    backtrack(candidates, target, i, currSum + candidates[i], subList, result);
    subList.pop();
  }
}
