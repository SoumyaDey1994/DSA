...
function findMaxAmountCanRobbed(
  input,
  start = 0,
  end = input.length - 1,
  result = new Map()
  ...
  const amountIncludingStart =
    input[start] + findMaxAmountCanRobbed(input, start + 2, end);
  const amountExcludingStart = findMaxAmountCanRobbed(input, start + 1, end);

  const amount = Math.max(amountIncludingStart, amountExcludingStart);
  result.set(start, amount);
  return amount;
}
