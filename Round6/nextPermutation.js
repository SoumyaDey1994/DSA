...
function findNextPermutation(input) {
  ...
  //No greater permutation exists, hence return the smallest permutation
  if (swapIndex === -1) return input.reverse();

  // reverse
  for (let i = swapIndex + 1, j = input.length - 1; i < j; i++, j--) {
    const temp = input[i];
    input[i] = input[j];
    input[j] = temp;
  }

  for (let i = swapIndex + 1; i <= input.length - 1; i++) {
    if (input[i] > input[swapIndex]) {
      // swap
      [input[i], input[swapIndex]] = [input[swapIndex], input[i]];
      break;
    }
  }
