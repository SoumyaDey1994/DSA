...
function findKthHeighestQuickSelect(nums, k) {
  if (!nums || nums.length === 0 || nums.length < k) return -1;
  ...
  if (pivotIndex < k) return findKthHeighestQuickSelect(right, k - pivotIndex);
  else return findKthHeighestQuickSelect(left, k);
}

console.log("....Quick-Select Approach....");
...
function findKthHeighestHeap(nums, k) {
  ...
  // Sort in ASC order
  const heapify = () => {
    return heap.sort((num1, num2) => num1 - num2);
  };
