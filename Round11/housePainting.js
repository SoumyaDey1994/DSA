...
function findWaysToPaintHouses(noOfHouses, noOfColors, firstColor, lastColor) {
  ...
  const startHouse = 2,
    prevHouseColor = firstColor;
  const ways = findWays(startHouse, noOfHouses, noOfColors, prevHouseColor, lastColor);
  return ways;
}

function findWays(houseNo, noOfHouses, noOfColors, prevHouseColor, lastColor) {
  if (houseNo === noOfHouses - 1) {
    return prevHouseColor === lastColor ? noOfColors - 1 : noOfColors - 2;
  }
  ...
  for (let i = 1; i <= noOfColors; i++) {
    if (i !== prevHouseColor) {
      count =
        count + findWays(houseNo + 1, noOfHouses, noOfColors, i, lastColor);
    }
  }
