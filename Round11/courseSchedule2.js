 ...
 */
function findOrderOfCourses(numCourses, prerequisites) {
  if (!numCourses || numCourses === 0) return;
  if (!prerequisites || prerequisites.length === 0) return;

  const courseDependencyCounts = new Array(numCourses).fill(0);
  const dependencyCourseMap = new Map();

  for (let [course, dep] of prerequisites) {
    courseDependencyCounts[course] = courseDependencyCounts[course] + 1;
  }
  ...
  const executionQueue = [],
    executionOrder = [];
  for (let i = 0; i < courseDependencyCounts.length; i++) {
    if (courseDependencyCounts[i] === 0) {
  ...
  while (executionQueue.length > 0) {
    ...
    const dependencies = dependencyCourseMap.get(currCourse);
    if (dependencies && dependencies.length > 0) {
      for (let dep of dependencies) {
        courseDependencyCounts[dep] = courseDependencyCounts[dep] - 1;
        if (courseDependencyCounts[dep] === 0) {
...
let outputOrder = findOrderOfCourses(numCourses, prerequisites);
console.log(
...
outputOrder = findOrderOfCourses(numCourses, prerequisites);
console.log(
  `Courses can be completed in following order: ${outputOrder.join("->")}`,
);
