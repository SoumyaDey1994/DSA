...
function removeDuplicates(nums) {
  ...
  for (let i = 0; i < nums.length; i++) {
    if (nums[i] !== nums[noOfUniques]) {
      ...
      const temp = nums[i];
      nums[i] = nums[noOfUniques];
      nums[noOfUniques] = temp;
    }
  }

  for (let k = noOfUniques + 1; k < nums.length; k++) {
    nums[k] = "_";
  }

  return { nums, noOfUniques: noOfUniques + 1 };
}

let nums = [1, 1, 2];
console.log(
  `Unique count & values are: ${JSON.stringify(removeDuplicates(nums))}`,
);
