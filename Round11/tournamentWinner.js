...
function findTournamentWinner(competitions, results) {
  if (competitions.length === 0 || results.length === 0) return null;

  const scoreCard = new Map();
  for (let i = 0; i < results.length; i++) {
    const [homeTeam, awayTeam] = competitions[i];
    if (!scoreCard.has(homeTeam)) {
      scoreCard.set(homeTeam, 0);
    }
    if (!scoreCard.has(awayTeam)) {
      scoreCard.set(awayTeam, 0);
    }
    const winner = results[i] === 1 ? homeTeam : awayTeam;
    scoreCard.set(winner, scoreCard.get(winner) + 3);
  }

  const hightestPoint = Math.max(...scoreCard.values());

  for (let team of scoreCard.keys()) {
    if (scoreCard.get(team) === hightestPoint) return team;
  }
