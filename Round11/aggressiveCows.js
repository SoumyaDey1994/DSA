...
function findMinDistance(stalls, cows) {
  ...
  if (stalls.length < cows) return 0;

  stalls = stalls.sort((s1, s2) => s1 - s2); // sort stalls in ASC order: O(nlogn)
  let min = 1,
    max = stalls[stalls.length - 1] - stalls[0];
  let result = -1;
  while (min <= max) {
    const pivot = Math.floor((min + max) / 2);
    const canBePlaced = canPlace(stalls, cows, pivot);
    if (canBePlaced) {
      result = pivot;
      min = pivot + 1;
    } else {
...
function canPlace(stalls, cows, pivot) {
