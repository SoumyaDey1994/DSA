 ...
 */
function findMinNoOfTrips(input, maxWeight) {
  if (!input || input.length === 0) return 0;

  input.sort((w1, w2) => w1 - w2);
  let left = 0,
    right = input.length - 1,
    tripCount = 0;

  while (left <= right) {
    const sumOfWeight = input[left] + input[right];
