 ...
 *      board = [
        ...
        word3: "WORLD"
            Output: false
 */
function isWordExists(board, targetWord) {
  ...
  for (let row = 0; row < rowCount; row++) {
    for (let col = 0; col < colCount; col++) {
      if (
        board[row][col] === targetWord[0] &&
        traverse(board, row, col, targetWord, 0)
      ) {
        return true;
      }
    }
...
function traverse(board, row, col, targetWord, wordIndex) {
  const rowCount = board.length,
    colCount = board[0].length;
  ...
  if (
    ...
    col < 0 ||
    col >= colCount ||
    board[row][col] !== targetWord[wordIndex]
  ) {
    return;
  }

  if (wordIndex === targetWord.length - 1) return true;

  const temp = board[row][col];
  board[row][col] = "#";

  const isExists =
    traverse(board, row - 1, col, targetWord, wordIndex + 1) ||
    traverse(board, row + 1, col, targetWord, wordIndex + 1) ||
    traverse(board, row, col - 1, targetWord, wordIndex + 1) ||
    traverse(board, row, col + 1, targetWord, wordIndex + 1);
