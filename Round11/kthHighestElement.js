 ...
 */
function findKthHighestElementQuickSelect(nums, k) {
  if (!nums || nums.length === 0 || k > nums.length) return null;
  ...
  if (pivotIndex === k) return pivot;
    ...
    return findKthHighestElementQuickSelect(left, k);
  } else {
    return findKthHighestElementQuickSelect(right, k - pivotIndex);
  }
}
...
let nums = [3, 1, 5, 12, 2, 8],
  k = 2;
let output = findKthHighestElementQuickSelect(nums, k);
console.log(`${k}th heighest element of [${nums}] is: ${output}`);

((nums = [7, 10, 4, 3, 20, 15]), (k = 3));
output = findKthHighestElementQuickSelect(nums, k);
console.log(`${k}th heighest element of [${nums}] is: ${output}`);
