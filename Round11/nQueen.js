...
function placeNQueen(n) {
  ...
  traverse(n, startRow, board, result);
  return result;
}

function traverse(n, startRow, board, result) {
  if (startRow === n) {
  ...
  for (let col = 0; col < n; col++) {
    const isQueenSafe = isSafe(n, startRow, col, board);
    if (isQueenSafe) {
      board[startRow][col] = "Q";
      traverse(n, startRow + 1, board, result);
      board[startRow][col] = ".";
    }
  }
}

function isSafe(n, row, col, board) {
