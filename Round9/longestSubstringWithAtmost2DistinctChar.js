 ...
 */
function findLengthOfSubstring(input, k = 2) {
  ...
  for (let right = 0; right < input.length; right++) {
    ...
    // remove the left-most character & increment left pointer
    if (indexMap.size > k) {
...
let input = "eceba";
console.log(
  `Longest Substring of ${input} with at most 2 char is of length: ${findLengthOfSubstring(
    input,
  )}`,
);
