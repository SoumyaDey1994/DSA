...
function findCombinationsForTargetSum(candidates, target) {
  ...
  const result = [];
  backtrack(candidates, target, 0, 0, [], result); // candidates, target, startIndex, currSum, subsets, result
  return result.map((comb) => `[${comb}]`);
}

function backtrack(candidates, target, startIndex, currSum, subsets, result) {
  if (currSum === target) {
    result.push([...subsets]);
    return;
  }
  ...
  for (let i = startIndex; i < candidates.length; i++) {
    ...
    backtrack(candidates, target, i, currSum + candidates[i], subsets, result);
    subsets.pop();
  }
}
