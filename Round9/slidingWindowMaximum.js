...
function getSlidingWindowMaxValues(nums, k) {
  ...
  for (let i = 0; i < nums.length; i++) {
    // remove indexes from beg which are out of bound
    if (indexQueue.length > 0 && indexQueue[0] < i - k + 1) {
      indexQueue.shift();
    }

    // remove inedes from length whose corresponding values < current value
    while (
      indexQueue.length > 0 &&
    ...
    indexQueue.push(i);
    // if curr > window, store max of curr window
    if (i + 1 >= k) {
      const targetIndex = indexQueue[0];
      output.push(nums[targetIndex]);
    }
  }
