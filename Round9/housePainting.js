...
function findWaysToPaintHouses(noOfHouses, noOfColors, firstColor, lastColor) {
  ...
  let startHouse = 1;
  // next house(2), noOfHouses, noOfColors, prevHouseColor, lastColor
  const noOfWays = track(
    startHouse + 1,
    noOfHouses,
    noOfColors,
    firstColor,
...
function track(houseNo, noOfHouses, noOfColors, prevColor, lastColor) {
  ...
  for (let i = 1; i <= noOfColors; i++) {
    if (prevColor !== i) {
      ways = ways + track(houseNo + 1, noOfHouses, noOfColors, i, lastColor);
    }
  }
