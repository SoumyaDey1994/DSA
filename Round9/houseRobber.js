 ...
 */
function findMaxAmountRobbed(houses) {
  if (!houses || houses.length === 0) return 0;

  function findMaxAmount(start, memo = new Map()) {
    if (start >= houses.length) return 0;
    if (start === houses.length - 1) return houses[start];

    const key = start;
    if (memo.has(key)) return memo.get(key);
    const amountIncludingStart = houses[start] + findMaxAmount(start + 2);
    const amountExcludingStart = findMaxAmount(start + 1);
  ...
  const maxAmount = findMaxAmount(0);
  return maxAmount;
}
