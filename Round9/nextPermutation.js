...
function findNextPermutation(input) {
  ...
  let swappedElementIndex = -1;
  // find index of element where change should happen
  for (let i = input.length - 1; i > 0; i--) {
    if (input[i] > input[i - 1]) {
      swappedElementIndex = i - 1;
      break;
    }
  }

  if (swappedElementIndex === -1) {
    return input.reverse();
  }
  // reverse remaining elements
  for (let j = swappedElementIndex + 1, k = input.length - 1; j < k; j++, k--) {
    const temp = input[j];
    input[j] = input[k];
    input[k] = temp;
  }
  // swap element[swappedElementIndex] with next immediate greater element
  for (let i = swappedElementIndex + 1; i < input.length; i++) {
    if (input[i] > input[swappedElementIndex]) {
      const temp = input[i];
      input[i] = input[swappedElementIndex];
      input[swappedElementIndex] = temp;
