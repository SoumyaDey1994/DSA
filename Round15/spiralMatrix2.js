 ...
 */
function generateSpiralMatrix(noOfRows) {
  if (!noOfRows || noOfRows === 0) return [];
  ...
  let top = 0,
    bottom = noOfRows - 1;
  let direction = 0;
  const output = new Array(noOfRows)
    .fill()
    .map(() => new Array(noOfRows).fill(null));

  while (left <= right && top <= bottom) {
    if (direction === 0) {
      // keep top same, move left --> right, incrm top
    ...
    if (direction === 1) {
      // keep right same, move top --> bottom, decr right
    ...
    if (direction === 2) {
      // keep bottom same, move right --> left, decr bottom
    ...
    if (direction === 3) {
      // keep left same, move bottom --> top, incr left
