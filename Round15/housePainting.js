...
function findWaysToPaintHouses(noOfHouses, noOfColors, firstColor, lastColor) {
  if (!noOfHouses || noOfHouses === 0) return 0;
  if (!noOfColors || noOfColors === 0) return 0;
...
function findWays(
  ...
  for (let k = 1; k <= noOfColors; k++) {
    if (k !== prevHouseColor) {
      ways =
        ways + findWays(noOfHouses, noOfColors, nextHouse + 1, k, lastColor);
    }
  }
