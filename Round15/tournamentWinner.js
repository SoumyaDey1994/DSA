...
function findTournamentWinner(competitions, results) {
  ...
  for (let i = 0; i < competitions.length; i++) {
    ...
    if (!scoreCard.has(homeTeam)) {
      scoreCard.set(homeTeam, 0); // team => point
    }

    if (!scoreCard.has(awayTeam)) {
      scoreCard.set(awayTeam, 0); // team => point
    }

    scoreCard.set(winningTeam, scoreCard.get(winningTeam) + 3);
  }

  const winningTeamPoints = Math.max(...scoreCard.values());

  for (const [team, _] of scoreCard.entries()) {
    if (scoreCard.get(team) === winningTeamPoints) {
      return team;
    }
  }
