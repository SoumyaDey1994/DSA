 ...
 */
function findMinCoinsRequired(coins, amount) {
  if (!coins || coins.length === 0) return;
  if (amount === 0) return 0;

  coins.sort((c1, c2) => c2 - c1); // sort coins by DESC order of value
...
let amount = 11;
console.log(
  `Min no of coins required to change amount ${amount} is: ${findMinCoinsRequired(
    coins,
    amount,
  )}`,
);
