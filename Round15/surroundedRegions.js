...
function getRegionPostSurrounding(grid) {
  ...
  for (let row = 0; row < rowCount; row++) {
    traverse(grid, row, 0); // first col
    traverse(grid, row, colCount - 1); // last col
  }

  for (let col = 0; col < colCount; col++) {
    traverse(grid, 0, col); // first row
    traverse(grid, rowCount - 1, col); // last row
  }
...
function traverse(grid, row, col) {
  ...
  grid[row][col] = "T"; //mark as visited
