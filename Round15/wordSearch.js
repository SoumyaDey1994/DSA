 ...
 *      board = [
        ...
        word3: "WORLD"
            Output: false
 */
function checkIfWordExists(board, targetWord) {
  ...
  for (let row = 0; row < rowCount; row++) {
    for (let col = 0; col < colCount; col++) {
      if (
        ...
        isExists(board, row, col, targetWord, 0)
      ) {
        return true;
      }
    }
...
function isExists(board, row, col, targetWord, index) {
  let rowCount = board.length,
    colCount = board[0].length;
  ...
  if (index === targetWord.length - 1) return true;

  const temp = board[row][col];
  board[row][col] = "#"; // mark as visited

  const isFound =
    isExists(board, row - 1, col, targetWord, index + 1) ||
    isExists(board, row + 1, col, targetWord, index + 1) ||
    isExists(board, row, col - 1, targetWord, index + 1) ||
    isExists(board, row, col + 1, targetWord, index + 1);
