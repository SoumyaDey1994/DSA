...
function findCommonCoordinates(matrix) {
  ...
  for (let row = 0; row < rowCount; row++) {
    traverse(matrix, pacific, row, 0); // left edge
    traverse(matrix, atlantic, row, colCount - 1); // right edge
  }

  for (let col = 0; col < colCount; col++) {
    traverse(matrix, pacific, 0, col); // top edge
    traverse(matrix, atlantic, rowCount - 1, col); // bottom edge
  }
...
function traverse(matrix, ocean, row, col) {
  ...
  ocean[row][col] = true; // mark as possible
