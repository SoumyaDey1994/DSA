 ...
 */
function findFrequencyInSortedList(arr, targetElement) {
  if (!arr || arr.length === 0) return;

  const leftIndex = findIndex(arr, targetElement, true);
  const rightIndex = findIndex(arr, targetElement, false);

  if (leftIndex > -1 && rightIndex > -1) return rightIndex - leftIndex + 1;
  return 0;
}

function findIndex(arr, targetElement, isLeft) {
  let left = 0,
    right = arr.length - 1;
...
let target = 2;
console.log(
  `Frequency of ${target} in [${arr}] is: ${findFrequencyInSortedList(arr, target)}`,
);
