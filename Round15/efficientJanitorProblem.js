 ...
 */
function findMinTrips(input, maxWeight) {
  if (!input || input.length === 0) return;

  input.sort((w1, w2) => w1 - w2); // sort weights in ASC order
  let left = 0,
    right = input.length - 1;
  let tripCount = 0;

  while (left < right) {
    const totalWeight = input[left] + input[right];
