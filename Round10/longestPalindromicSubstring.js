...
function findLongestPalindromeSubstr(inputStr) {
  ...
  function getLongestPalindome(str, start, end) {
    if (start > end) return "";
    if (start === end) return str[start];

    if (str[start] === str[end]) {
      const innerPlaindrome = getLongestPalindome(str, start + 1, end - 1);
      if (innerPlaindrome.length === end - start - 1) {
        return str[start] + innerPlaindrome + str[end];
      }
    }

    const palindromeIncStart = getLongestPalindome(str, start, end - 1);
    const palindromeExclStart = getLongestPalindome(str, start + 1, end);

    return palindromeIncStart.length > palindromeExclStart.length
      ? palindromeIncStart
      : palindromeExclStart;
  }

  return getLongestPalindome(inputStr, 0, inputStr.length - 1);
}
