...
function findMinTimeToRotAllMangoes(grid) {
  ...
  for (let row = 0; row < rowCount; row++) {
    for (let col = 0; col < colCount; col++) {
      if (grid[row][col] === 2) {
        executionQueue.push([row, col, 0]); //row, col, time
      }
  ...
  while (executionQueue.length > 0) {
    const [currRow, currCol, time] = executionQueue.shift();

    if (countOfFreshMangoes === 0) return time;

    if (currRow > 0 && grid[currRow - 1][currCol] === 1) { // top
      countOfFreshMangoes--;
      grid[currRow - 1][currCol] = 2; //mark as rotten
      executionQueue.push([currRow - 1, currCol, time + 1]);
    }

    if (currCol > 0 && grid[currRow][currCol - 1] === 1) { // left
      countOfFreshMangoes--;
      grid[currRow][currCol - 1] = 2; //mark as rotten
      executionQueue.push([currRow, currCol - 1, time + 1]);
    }

    if (currRow < rowCount - 1 && grid[currRow + 1][currCol] === 1) { // bottom
      countOfFreshMangoes--;
      grid[currRow + 1][currCol] = 2; //mark as rotten
      executionQueue.push([currRow + 1, currCol, time + 1]);
    }

    if (currCol < colCount - 1 && grid[currRow][currCol + 1] === 1) { // right
      countOfFreshMangoes--;
      grid[currRow][currCol + 1] = 2; //mark as rotten
      executionQueue.push([currRow, currCol + 1, time + 1]);
    }
...
console.log(
  `No of mins required to rot all mangoes in grid: ${findMinTimeToRotAllMangoes(grid)}`,
);
