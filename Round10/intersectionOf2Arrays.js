...
function findIntersection(nums1, nums2) {
  ...
  const nums1FreqMap = new Map();
  for (const num of nums1) {
    nums1FreqMap.set(num, (nums1FreqMap.get(num) || 0) + 1);
  }

  const result = [];
  for (const num of nums2) {
    if (nums1FreqMap.has(num)) {
      result.push(num);
      if (nums1FreqMap.get(num) === 1) {
        nums1FreqMap.delete(num);
      } else {
        nums1FreqMap.set(num, nums1FreqMap.get(num) - 1);
      }
    }
  }
