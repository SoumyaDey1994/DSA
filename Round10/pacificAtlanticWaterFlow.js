...
function findCommonCoordinates(matrix) {
  ...
  for (let row = 0; row < rowCount; row++) {
    traverse(matrix, pacific, row, 0); // left col: pacific
    traverse(matrix, atlantic, row, colCount - 1); // right col: atlantic
  }

  for (let col = 0; col < colCount; col++) {
    traverse(matrix, pacific, 0, col); // top row: pacific
    traverse(matrix, atlantic, rowCount - 1, col); // bottom row: atlantic
  }
