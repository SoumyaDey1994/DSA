...
function searchElementRecursive(nums, target) {
  ...
  function findIndex(low, high) {
    if (low > high) return -1;

    const mid = Math.floor((low + high) / 2);
    if (nums[mid] === target) return mid;
    else if (target < nums[mid]) return findIndex(low, mid - 1);
    else return findIndex(mid + 1, high);
  }
...
let nums = [-3, -1, 0, 3, 5, 9, 12],
  target = 9;
console.log(
  `Index of element ${target} in [${nums}] is: ${searchElement(nums, target)}`,
);
