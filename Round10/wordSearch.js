...
function traverse(board, row, col, targetWord, charIndex) {
  const rowCount = board.length,
    colCount = board[0].length;
  if (
    row < 0 ||
    ...
    row >= rowCount ||
    col >= colCount ||
    board[row][col] !== targetWord[charIndex]
  )
    return;

  if (charIndex === targetWord.length - 1) return true;

  const temp = board[row][col];
  board[row][col] = "#";

  const isFound =
    traverse(board, row - 1, col, targetWord, charIndex + 1) ||
    traverse(board, row + 1, col, targetWord, charIndex + 1) ||
    traverse(board, row, col - 1, targetWord, charIndex + 1) ||
    traverse(board, row, col + 1, targetWord, charIndex + 1);
