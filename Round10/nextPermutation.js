...
function findNextPermutation(input) {
  ...
  if (indexOfChange === -1) return input.reverse();

  // reverse remaining array numbers
  for (j = indexOfChange + 1, k = input.length - 1; j < k; j++, k--) {
    const temp = input[j];
    input[j] = input[k];
    input[k] = temp;
  }

  // swap with immediate greater element
  for (let i = indexOfChange + 1; i < input.length; i++) {
    if (input[i] > input[indexOfChange]) {
      const temp = input[indexOfChange];
      input[indexOfChange] = input[i];
      input[i] = temp;
      break;
    }
  }
