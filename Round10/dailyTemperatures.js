...
function findWaitingDays(temperatues) {
  ...
  const output = new Array(temperatues.length).fill(0);
  const stack = [];

  for (let index = 0; index < temperatues.length; index++) {
    while (
      stack.length > 0 &&
      temperatues[index] > temperatues[stack[stack.length - 1]]
    ) {
      const lastTempIndex = stack.pop();
      output[lastTempIndex] = index - lastTempIndex;
    }
    stack.push(index);
  }
