 ...
 */
function findLenthOfUniqueElements(nums) {
  if (!nums || nums.length === 0) return 0;

  let countOfUnique = 0;
  for (let i = 0; i < nums.length; i++) {
    if (nums[i] !== nums[countOfUnique]) {
      countOfUnique++;

      const temp = nums[i];
      nums[i] = nums[countOfUnique];
      nums[countOfUnique] = temp;
    }
  }

  // for(let i=countOfUnique+1; i<nums.length; i++){
  //     nums[i] = '_';
  // }

  // console.log(nums);
  return countOfUnique + 1;
}

let nums = [1, 1, 2];
console.log(
  `Unique count & elements of [${nums}] are: ${JSON.stringify(
    findLenthOfUniqueElements(nums),
  )}`,
);
...
nums = [5, 7, 7, 9, 9, 9, 9, 11, 12, 12, 13, 15, 15, 15, 15, 15];
console.log(
  `Unique count & elements of [${nums}] are: ${JSON.stringify(
    findLenthOfUniqueElements(nums),
  )}`,
