...
function findCoordinates(matrix) {
  ...
  for (let col = 0; col < colCount; col++) {
    traverse(matrix, pacific, 0, col); // top
    traverse(matrix, atlantic, rowCount - 1, col); //bottom
  }

  // left & right edges
  for (let row = 0; row < rowCount; row++) {
    traverse(matrix, pacific, row, 0); //left
    traverse(matrix, atlantic, row, colCount - 1); //right
  }
