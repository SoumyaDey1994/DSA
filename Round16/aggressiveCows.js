 ...
 *                   This is the maximum possible minimum distance.
 */
function findMinDistance(stalls, noOfCows) {
  if (!stalls || stalls.length === 0 || noOfCows === 0) return;

  stalls.sort((s1, s2) => s1 - s2);
  let min = 1,
    max = stalls[stalls.length - 1] - stalls[0];
  let outputDist = 0;
  while (min <= max) {
    const pivot = Math.floor((min + max) / 2);
    const isPossible = isPossibleToPlaceCows(stalls, noOfCows, pivot);
    if (isPossible) {
      outputDist = pivot;
      min = pivot + 1;
    } else {
...
function isPossibleToPlaceCows(stalls, noOfCows, pivot) {
  let cowCount = 1,
    lastPlaced = stalls[0];

  for (let i = 1; i < stalls.length; i++) {
    const curr = stalls[i];
    if (curr - lastPlaced >= pivot) {
      lastPlaced = curr;
      cowCount++;
    }
  }

  return cowCount === noOfCows;
}
