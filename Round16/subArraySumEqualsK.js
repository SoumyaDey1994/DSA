 ...
 *          [1,4,2] (index 5 to 7)
 */
function findNoOfSubArrays(numbers, targetSum) {
  if (!numbers || numbers.length === 0) return;

  let sumMap = new Map();
  sumMap.set(0, 1); // only 1 possible subarray with sum=0 is []

  let subArrayCount = 0,
    currSum = 0;
  for (let i = 0; i < numbers.length; i++) {
    currSum = currSum + numbers[i];
    const complement = currSum - targetSum;
    if (sumMap.has(complement)) {
      subArrayCount = subArrayCount + sumMap.get(complement);
    }
