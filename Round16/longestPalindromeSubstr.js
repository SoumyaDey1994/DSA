...
function findLongestPalindromeSubstr(input) {
  ...
  function findSubStr(start, end) {
    ...
    const subStrExcludingStart = findSubStr(start + 1, end);
    const subStrExcludingEnd = findSubStr(start, end - 1);
    const result =
      subStrExcludingStart.length > subStrExcludingEnd.length
  ...
  return findSubStr(0, input.length - 1);
}
