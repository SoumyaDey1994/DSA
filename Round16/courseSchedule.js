...
function isPossibleToFinishCourses(numCourses, prerequisites) {
  ...
  while (courseExecutionQueue.length > 0) {
    ...
    if (depCourses && depCourses.length > 0) {
      for (let cr of depCourses) {
        courseDepenedencyCounts[cr] = courseDepenedencyCounts[cr] - 1;
        if (courseDepenedencyCounts[cr] === 0) {
          courseExecutionQueue.push(cr);
        }
      }
    }
  }
