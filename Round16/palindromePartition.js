...
function getPalindromePartitions(inputStr) {
  ...
  const subStrings = [],
    result = [],
    startIndex = 0;
  backtrack(inputStr, startIndex, subStrings, result);
  return result.map((group) => `[${group}]`);
}

function backtrack(inputStr, startIndex, subStrings, result) {
  ...
  for (let i = startIndex; i < inputStr.length; i++) {
    ...
    if (isPalindrome) {
      subStrings.push(inputStr.slice(startIndex, i + 1));
      backtrack(inputStr, i + 1, subStrings, result);
      subStrings.pop();
    }
  }
}
