...
function findWaysToPaintHouses(noOfHouses, noOfColors, firstColor, lastColor) {
  ...
  const prevHouseColor = firstColor,
    lastHouseColor = lastColor;
  const startingHouse = 2;
  const noOfWays = findWays(
    startingHouse,
    noOfHouses,
    noOfColors,
    prevHouseColor,
...
function findWays(
  ...
  if (startingHouse === noOfHouses - 1) {
    return prevHouseColor === lastHouseColor ? noOfColors - 1 : noOfColors - 2;
  }

  let wayCount = 0;
  for (let k = 1; k <= noOfColors; k++) {
    if (k !== prevHouseColor) {
      wayCount =
        wayCount +
        findWays(startingHouse + 1, noOfHouses, noOfColors, k, lastHouseColor);
    }
  }
