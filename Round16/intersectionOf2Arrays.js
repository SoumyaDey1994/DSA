 ...
 */
function findIntersection(numbers1, numbers2) {
  if (!numbers1 || numbers1.length === 0) return [];
  if (!numbers2 || numbers2.length === 0) return [];
  ...
  for (const num of numbers2) {
    if (freqMap.get(num) > 0) {
      commonList.push(num);
      const freqOfNum = freqMap.get(num);
      if (freqOfNum === 1) {
        freqMap.delete(num);
      } else {
        freqMap.set(num, freqOfNum - 1);
      }
    }
  }
...
((nums1 = [1, 3, 5, 6, 7]), (nums2 = [2, 4, 7, 8, 9]));
console.log(
  `Intersection of [${nums1}] & [${nums2}] is: [${findIntersection(
    nums1,
    nums2,
