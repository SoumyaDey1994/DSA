 ...
 *
 */
function findLargesstMinDistance(stalls, cows) {
  if (cows === 0) return 0;
  if (!stalls || stalls.length === 0) return 0;

  // sort the stalls by order
  stalls.sort((st1, st2) => st1 - st2); // O(nlogn)

  let min = 1;
  let max = stalls[stalls.length - 1] - stalls[0];

  let result = -1;
  while (min <= max) { // O(logn)
    const pivot = Math.floor((min + max) / 2);
    // console.log(pivot)
    if (canBePlaced(stalls, cows, pivot)) {
      result = pivot;
      min = pivot + 1;
    } else {
      max = pivot - 1;
...
function canBePlaced(stalls, cows, pivot) {
  let count = 1,
    lastPlaced = stalls[0];
  for (let i = 1; i < stalls.length; i++) { // O(n)
...
let stalls = [1, 2, 8, 4, 9],
  cows = 3;
console.log(
  `Minimum distance among ${cows} cows should be: ${findLargesstMinDistance(
    stalls,
    cows,
  )}`,
);
