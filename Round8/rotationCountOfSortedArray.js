...
function findRotationCount(input) {
  ...
  while (left <= right) {
    ...
    if (input[pivot] < input[pivot - 1] && input[pivot] < input[pivot + 1]) {
      return pivot;
    } else if (input[right] < input[pivot]) {
      // right < pivot, means the target point lies b/w pivot & right
      left = pivot + 1;
    } else {
      // right > pivot, means right side sorted & target point lies b/w left & pivot
      right = pivot - 1;
    }
  }
