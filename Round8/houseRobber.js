 ...
 */
function findMaxAmountCanBeRobbed(input) {
  if (!input || input.length === 0) return 0;

  function getAmount(input, start, memo) {
    if (start >= input.length) return 0;
    if (start === input.length - 1) return input[start];

    if (memo.has(start)) return memo.get(start);

    const amountIncludingStart =
      input[start] + getAmount(input, start + 2, memo);
    const amountExcludingStart = getAmount(input, start + 1, memo);

    const result = Math.max(amountExcludingStart, amountIncludingStart);
    memo.set(start, result);
    return result;
  }

  const memo = new Map();
  const amount = getAmount(input, 0, memo);

  return amount;
}

let amounts = [2, 7, 9, 3, 1];
let maxAmount = findMaxAmountCanBeRobbed(amounts);
console.log(`Maximum amount can be robbed from [${amounts}] is: ${maxAmount}`);

amounts = [5, 1, 1, 10];
maxAmount = findMaxAmountCanBeRobbed(amounts);
console.log(`Maximum amount can be robbed from [${amounts}] is: ${maxAmount}`);
