...
function findTotalTrappedWaterVolume(heights) {
  ...
  while (left < right) {
    if (maxLeft < maxRight) {
      ...
      total = total + Math.max(0, maxLeft - heights[left]);
    } else {
      right--;
      maxRight = Math.max(maxRight, heights[right]);
      total = total + Math.max(0, maxRight - heights[right]);
    }
  }
...
let output = findTotalTrappedWaterVolume(heights);
console.log(`Total amount of water can be captured: ${output}`);

heights = [3, 3, 3, 3, 3];
output = findTotalTrappedWaterVolume(heights);
console.log(`Total amount of water can be captured: ${output}`);
