...
function findMinTimeToRotAllMangoes(grid) {
  ...
  for (let row = 0; row < rowCount; row++) {
    for (let col = 0; col < colCount; col++) {
      if (grid[row][col] === 2) {
        queue.push([row, col, 0]); // starting point
      }
  ...
  while (queue.length > 0) {
    ...
    // console.log(`Row: ${currRow}, Col: ${currCol}, time: ${time}, count: ${countOfFreshMangoes}`);

    if (countOfFreshMangoes === 0) return time; // stop condition

    if (currRow > 0 && grid[currRow - 1][currCol] === 1) {
      countOfFreshMangoes--;
      grid[currRow - 1][currCol] = 2; // mark as rotten
      queue.push([currRow - 1, currCol, time + 1]); // top
    }

    if (currRow < rowCount - 1 && grid[currRow + 1][currCol] === 1) {
      countOfFreshMangoes--;
      grid[currRow + 1][currCol] = 2; // mark as rotten
      queue.push([currRow + 1, currCol, time + 1]); // bottom
    }

    if (currCol > 0 && grid[currRow][currCol - 1] === 1) {
      countOfFreshMangoes--;
      grid[currRow][currCol - 1] = 2; // mark as rotten
      queue.push([currRow, currCol - 1, time + 1]); // left
    }

    if (currCol < colCount - 1 && grid[currRow][currCol + 1] === 1) {
      countOfFreshMangoes--;
      grid[currRow][currCol + 1] = 2; // mark as rotten
      queue.push([currRow, currCol + 1, time + 1]); // right
    }
  }
