 ...
 */
function findMinTrips(input, maxCapacity) {
  if (!input || input.length === 0) return 0;

  input.sort((a, b) => a - b); // sort weights in ASC order
  ...
  while (left <= right) {
    const total = input[left] + input[right];
...
console.log(
  `No of trips required to carry [${bags}] bags are: ${findMinTrips(bags, maxWeight)}`,
);
