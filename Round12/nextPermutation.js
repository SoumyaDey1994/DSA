...
function findNextPermutation(input) {
  ...
  // no greater exists, return reverse of current state
  if (targetIndex === -1) {
    return input.reverse();
  }
  // reverse tail portion of input
  for (let i = targetIndex + 1, j = input.length - 1; i < j; i++, j--) {
    const temp = input[i];
    input[i] = input[j];
    input[j] = temp;
  }

  // find immediate greater at right & swap
  for (let i = targetIndex + 1; i < input.length; i++) {
    if (input[i] > input[targetIndex]) {
      const temp = input[i];
      input[i] = input[targetIndex];
