...
function findCommonCoordinates(matrix) {
  ...
  for (let col = 0; col < colCount; col++) {
    traverse(matrix, pacific, 0, col); // top row for pacific
    traverse(matrix, atlantic, rowCount - 1, col); // botton row for atlantic
  }

  for (let row = 0; row < rowCount; row++) {
    traverse(matrix, pacific, row, 0); // left col for pacific
    traverse(matrix, atlantic, row, colCount - 1); // right col for atlantic
  }
