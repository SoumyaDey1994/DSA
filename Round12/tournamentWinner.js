...
function findTournamentWinner(competitions, results) {
  ...
  if (competitions.length !== results.length) return null;
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    pointsMap.set(winner, pointsMap.get(winner) + 1);
  }

  const MaxPointAchievedByTeam = Math.max(...pointsMap.values());

  for (let team of pointsMap.keys()) {
    if (pointsMap.get(team) === MaxPointAchievedByTeam) {
      return team;
    }
  }
