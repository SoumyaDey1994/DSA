...
function findMaxAmountCanBeRobbed(input) {
  ...
  function findAmount(start) {
    if (start >= input.length) return 0;
    if (start === input.length - 1) return input[start];

    if (memo.has(start)) return memo.get(start);

    const amountIncludigStart = input[start] + findAmount(start + 2);
    const amountExcludingStart = findAmount(start + 1);
    const result = Math.max(amountIncludigStart, amountExcludingStart);
