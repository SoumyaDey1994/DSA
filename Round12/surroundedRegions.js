...
function getSurroundedRegions(matrix) {
  ...
  for (let row = 0; row < rowCount; row++) {
    traverse(matrix, row, 0); // first col
    traverse(matrix, row, colCount - 1); //last col
  }

  for (let col = 0; col < colCount; col++) {
    traverse(matrix, 0, col); // first row
    traverse(matrix, rowCount - 1, col); // last row
  }
...
function traverse(matrix, row, col) {
  ...
  matrix[row][col] = "T"; //mark as visited temporarily
